...
public class DBPirate {
    ...
    public static Captain getPiratesCaptain(Pirate pirate){
        ...
        try{
            Criteria criteria = session.createCriteria(Captain.class);
            criteria.add(Restrictions.eq("ship", pirate.getShip()));
            captain =(Captain)criteria.uniqueResult();
        }catch (HibernateException e) {
            transaction.rollback();
            e.printStackTrace();
        } finally {
    ...
    public static List<Raid> getPiratesOnRaid(Pirate pirate){
        session = HibernateUtil.getSessionFactory().openSession();
        List<Raid> results = null;
        try{
            Criteria criteria = session.createCriteria(Raid.class);

            criteria.createAlias("pirates", "pirate");
            criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);
            criteria.add(Restrictions.eq("pirate.id", pirate.getId()));
